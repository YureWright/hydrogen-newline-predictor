/** 技术原理 · 双 Tab（机器学习 / 物理模型）—— HydrogenHowItWorks 与文档库共用 */
  ...
  return (
    <>
          ...
          <Sec num="2" title="数据：来源与处理">
            ...
            <Sub title="2.4 段聚合">
              <p>按行程（相邻时间差 &gt;300s 视为新行程）把 60s 点按 5km 累计里程分桶（`sid=int(cum/5)`，段长≈5km±一个点距）→ <b>1066 个路段样本</b>（两车混合，过滤 h2_per_km∈(0.02,0.5) 且 len_km≥1 后）。每段统计：均速/坡度/海拔/温度/风速/湿度用<b>里程加权平均</b>（每点按它代表的里程加权，避免低速点灌水、与预测端高德整段均速口径对齐）、<b>爬升密度</b>（段内正海拔差求和÷里程，补平均坡度丢失的段内起伏）、道路等级取众数、时段取段首点、段长为距离求和、目标 kg/km 为总氢耗÷总里程。</p>
            </Sub>
            <Sub title="2.5 训练 vs 预测的数据来源口径差异（透明披露）">
              <Table head={["特征","训练来源（ml/backfill.py）","预测来源（前端实时抓取）"]} rows={[
                ...
                ["纵向风分量 wind_par","ERA5 风向（wind_direction_10m）+ 实车 GPS 段首尾航向，w·cos(φ−θ) 里程加权","QWeather wind360 / 高德文字转角度 + 段首尾方位角，同公式同口径"],
...
ho C_dA v_i^3\Delta t_i" />
          ...
          <Sec num="3" title="特征工程（21 维，两级）">
            <Sub title="3.1 第一级：新线路可直接获取（12 个）">
              <Table head={["特征","含义","来源"]} rows={[
                ["len_km / v_mean","段长 km / 段均速 km/h","高德路线"],
                ["grade_mean / elev_mean","平均坡度 % / 平均海拔 m","DEM"],
                ["gain_m_per_km","爬升密度 m/km（段内总爬升÷里程，补平均坡度丢失的段内起伏）","DEM"],
                ["temp_mean / wind_mean / hum_mean","温度℃ / 风速km/h / 湿度%","天气(ERA5/QWeather)"],
                ["wind_par","纵向风分量 km/h = 风速×cos(风向角−航向角)，逆风正/顺风负（相对车头）","训练：ERA5 风向+段首尾航向；预测：QWeather 风向/高德文字转角度+段首尾方位角"],
                ["hour / lv","到达时段 / 道路等级序数(高速0…其他6)","出发时间 / OSM·高德"],
              ]} />
            </Sub>
            <p className="hw-note">2026-08-23 实测：新增纵向风分量 wind_par（段级航向口径，训练/预测一致）后，按行程分组 CV R² 0.382 → <b>0.385</b>（RMSE 0.0473→0.0474）；横向分量 wind_perp（侧风）实测无提升（0.385→0.381）已放弃——与物理预期一致（侧风对直行风阻影响小，信号弱于噪声）。风向两套数据（QWeather/ERA5 数字角度 vs 高德文字）已统一为数字角度（16 方位映射，见 weather.ts）。</p>
            ...
            <Sub title="3.3 工况合成（核心创新）">
              ...
              <div className="hw-story">
                <b>👵 通俗理解（老奶奶版）：这到底是在干嘛？</b>
                <p>给一条<b>新路</b>算氢耗，但我们只知道它的平均速度（比如 70）。光知道均速不够——同样开 70，一路匀速巡航 vs 走走停停，氢耗能差 20~50%。可新路没法实跑，只能<b>「演」出来</b>：</p>
                <ul>
                  <li><b>素材（片段库）</b>：把两辆实车的 60s 速度点按「道路等级 × 均速」分进 <b>8 个抽屉</b>，每个抽屉装着司机在这类路上的真实踩油门习惯。</li>
                  <li><b>① 拼</b>：从对应抽屉随机抓几段速度片段接成一条速度线——像从几盘录像带里剪片段拼一段模拟开车。</li>
                  <li><b>② 调</b>：把拼好的线整体缩放，让平均速度正好等于新路的均速（像按 0.9 / 1.1 倍速播放录像）。</li>
                  <li><b>③ 上坡减速</b>：有坡度的地方速度再调一下：上坡 <Tex math="v\leftarrow v(1-0.025g)" />、下坡略快（像爬山自然走慢、下坡走快）。</li>
                  <li><b>④ 加速度</b>：速度相邻两帧差值 ÷ 时间，就是这秒踩油门还是踩刹车、多猛。</li>
                  <li><b>⑤ 体检</b>：从这段演出来的曲线数出 9 个指标（速度稳不稳、实际巡航多快、刹车多猛、停车多久、每公里加速/空阻/上坡花多少能量）——就是喂给模型的那 9 个深度特征。</li>
                </ul>
                <p><b>一句话</b>：把实车司机在这类路上怎么踩油门的习惯，给新线路演一遍，再从戏里数出 9 个驾驶体检指标——让模型能区分匀速省着开和走走停停费着开。实测这套方法把段级 R² 从 0.31 提到 0.45+，是模型提升最大的环节。</p>
              </div>
          ...
          <Sec num="4" title="模型选择与训练">
            <Sub title="4.1 为什么选 HistGB（直方图梯度提升树）">
              ...
              <p><b>2026-08-22 模型实测对比</b>（`ml/compare_models.py`，与训练同管道、按行程分组 5 折 CV）：</p>
              <Table head={["模型","R²","RMSE"]} rows={[
                ["HistGB（采用）","0.382","0.0473"],
                ["RandomForest","0.370","0.0487"],
                ["GBR","0.336","0.0497"],
                ["Ridge / Lasso / ElasticNet（标准化）","0.25~0.26","0.0526~0.0529"],
                ["SVR-RBF / MLP","≤0 / 负","0.063~0.082"],
                ["Stacking(hist+gbr+ridge)","0.380","0.0487"],
              ]} />
              <p>结论：① 当前 20 特征 <b>VIF 实测平均 226、最大 2106</b>（v_mean 与 v_p85 强相关、深度特征为速度/加速度衍生），
              共线性仍高——树模型免疫共线性的优势成立，线性模型被拖累（R² 仅 0.25）；② <b>HistGB 是最优单模型</b>；
              ③ <b>Stacking 集成无提升</b>（0.380 &lt; 0.382）且更复杂，故不采用。保留 HistGB（简单、可解释、CV 稳定）。</p>
            </Sub>
          ...
          <Sec num="5" title="预测部署：输入、来源与兜底">
            <Sub title="5.1 预测输入清单（每段）">
              <Table head={["特征","在线来源","缺失时兜底"]} rows={[
                ...
                ["wind_par 纵向风分量","风速×cos(风向角−航向角)：风向 QWeather wind360 / 高德文字转角度；航向=段首尾方位角","默认 0（风向列显示真实值时可用）"],
                ["hum_mean","同上","默认 60%"],
          ...
          <Sec num="6" title="结果与评估（实测）">
            <Sub title="6.1 交叉验证">
              <p><b>按行程分组 5 折 CV：R²=0.385（每折 R² 取平均），RMSE=0.047 kg/km</b>（目标中位 ≈5.12 kg/100km；2026-08-22 修正 haversine + 质量特征 + 里程加权聚合，2026-08-23 新增纵向风分量 wind_par 后重训 0.382→0.385；口径说明见 §6.4 末注）。此值使用真实 v 差分加速度，为诚实水平。</p>
            </Sub>
            ...
            <Sub title="6.4 校准评估与移除（2026-08-22 实测）">
              <p>模型输出是每段的条件<b>期望值（均值）</b>，而氢耗目标右偏（长尾），曾据此拟合分段常数校准，把预测拉回"中位水平"（低速扣 0.76~0.96、高速扣 0.50 kg/100km）。</p>
              <p><b>但训练数据 CV 实测显示校准没有提升</b>（同一批 OOF 预测）：</p>
              <Table head={["校准方式","整体 OOF R²","RMSE","MAE"]} rows={[
                ["无校准","0.4780","0.0477","0.0284"],
                ["中位偏差校准（曾用）","0.4765","0.0478","0.0280"],
                ["均值偏差校准","0.4793","0.0476","0.0282"],
              ]} />
              <p>原因：模型在<b>均值意义下已很准</b>（各均速段均值偏差仅 +0.03~0.37 kg/100km），而校准减的是<b>中位偏差</b>（0-40 段达 +1.89），把预测拉向中位反而偏离了 RMSE/R² 最优的均值水平。因此<b>已移除校准</b>，预测直接使用模型原始输出。</p>
              <p className="hw-note">口径说明：本页 R² 默认"按行程分组 5 折 CV、每折 R² 取平均"（≈0.38，保守口径）；同一批预测拼起来算整体 OOF R² 约 0.48，两种算法不同、不可直接比较。模型对比见 §4.1，校准实测见 WORKLOG。</p>
            </Sub>
          </Sec>
...
<Sec num="8" title="局限与展望">
          ...
          <div className="hw-story">
            <b>📌 定位与说明（诚实披露）</b>
            <p>受<b>数据局限</b>（企业原始 h2_remain 毛刺占比 &gt;99.9%、清洗后目标仍系统性低估约 2 倍，详见 docs/data-quality-audit.md）与<b>时间有限</b>的影响，
            当前机器学习氢耗预测属于<b>探索性尝试</b>——完整跑通了「数据 → 特征工程 → 工况合成 → 训练 → 预测 → 前端」全链路，证明了技术方案的可行性，
            但预测精度有限（按行程分组 CV R²≈0.38）。</p>
            <p>后续若获得<b>更多高质量数据</b>（可靠的储氢计量 / 载重标签 / 秒级采集），将持续深入研究，优化特征与模型，预测效果会进一步提升。</p>
          </div>
    </>
  )
}
  ...
  return (
    <>
            ...
            <Sub title="7.3 车型预设（海珀特全系）与物理参数">
              <p>前端「🚛 车型选择」预设海珀特三款燃料电池车型，一键切换；物理模型按所选车型逐段计算（<code>ml/physics.py</code> 从每段读 <code>crr / cd / frontArea / eta_mt / p_fc_min / p_fc_max / p_bat_max / eta_fc / p_aux0 / k_t / massKg</code>，未传则回落 H49 默认值）。</p>
              <Table head={["车型","整备/总质量","燃料电池","电池","风阻 Cd / 迎风 A","官耗锚点","官网数据源"]} rows={[
                ["H49 · 49t 半挂牵引车（干线）","9.7t / 49t","300 kW","75 kWh","0.35 / 7.5 m²","7.1 kg/100km（满载高速）","hybot.com.cn/H49.html"],
                ["H18 · 18t 厢式运输车（城际）","9.8t / 18t","120 kW","50 kWh","≈0.55* / ≈8.5 m²*","&lt;5.5 kg/100km","hybot.com.cn/Product/2.html"],
                ["H4.5 · 4.5t 冷藏车（冷链）","3.7t / 4.5t","80/90 kW","14.9 kWh","≈0.55* / ≈6.0 m²*","&lt;3.10 kg/100km","hybot.com.cn/Product/3.html"],
              ]} />
              <p className="hw-note">* 打星参数官网未公布：Cd / 迎风面积按同族厢式/冷藏车常规值估算（H18 由外廓 2.54×3.95m 推导），界面已如实标注，可在「高级参数」手动修正。除车型预设外，所有物理参数（Crr/Cd/A/η_mt/电堆上下限/电池限幅/η_fc/附件基础功率与温度系数/整备质量）均可在前端调整，调整后立即作用于后续测算。</p>
              <Table head={["参数","H49","H18","H4.5","来源"]} rows={[
                ["滚动阻力系数 C_rr","0.009","0.009","0.009","重载/轻卡典型值"],
                ["风阻系数 C_d","0.35","0.55*","0.55*","H49 官方；其余估算"],
                ["迎风面积 A","7.5 m²","8.5 m²*","6.0 m²*","Class 8 / 外廓推导"],
                ["电机+传动效率 η_mt","0.90","0.90","0.90","永磁同步+AMT 典型"],
                ["电堆功率区间 P_fc","30~180 kW","12~72 kW","9~54 kW","额定×[10%,60%]"],
                ["电池功率限幅 P_bat","±150 kW","±100 kW","±30 kW","容量×2C 持续"],
                ["电堆系统效率 η_fc","0.50","0.50","0.50","官方 &gt;55%，取保守值"],
                ["氢低热值 LHV","33.3 kWh/kg","33.3","33.3","120 MJ/kg ÷ 3.6"],
              ]} />
            </Sub>
          ...
          <Sec num="8" title="临界角（等效摩擦角）：什么时候下坡白捡、什么时候要驱动">
            <p>把坡度阻力与滚动阻力比一比，会冒出一个临界坡度（Gillespie 1992；余志生《汽车理论》）：</p>
            <Tex block math="\theta_c \;=\; \arctan(C_{rr}) \;\approx\; 0.52^\circ \;\text{（约 0.9% 坡度）}" />
            <Table head={["下坡坡度","重力沿坡分量 vs 滚动阻力","车会怎样","模型怎么处理"]} rows={[
              ["0 &lt; θ &lt; θ_c（缓下坡）","重力 &lt; 滚动阻力","仍需轻微驱动","正常按阻力公式算：P_drive &gt; 0，电堆做功（高速缓下坡 P_fc 完全跟随需求）"],
              ["θ = θ_c","重力 ≈ 滚动阻力","松油门能匀速滑行","轮端功率 ≈ 0"],
              ["θ &gt; θ_c（陡下坡）","重力 &gt; 滚动阻力","越滑越快，必须制动","先再生回收：P_wheel &lt; 0 → 电堆最低 30kW、回收充电池"],
            ]} />
            <p className="hw-note">代码实现：临界角<b>不需要单独开关</b>——每段算 <Tex math="F_{total}=F_{roll}+F_{aero}+F_{grade}" />，由 <Tex math="P_{wheel}=F_{total}\,v" /> 的符号天然判断：正=需驱动（电堆出力），负=净回收（再生）。实测 -0.5% 缓下坡 80km/h 时 P_wheel=+43.6kW（电堆 51.5kW 做功）；-3% 陡下坡时 P_wheel=−119.7kW（再生）。</p>
          </Sec>

          <Sec num="9" title="附件功耗：明细与温度耦合（不是常数）">
            <Table head={["附件","功率","处理"]} rows={[
              ["驾驶室空调（制冷 / PTC 制热）","0~8 kW，随温度（P_aux 主因）","进 P_aux"],
              ["气泵（制动）/ 转向泵 EHPS","1~3 kW","进 P_aux"],
              ["燃料电池 BOP（空压机/氢循环泵/电堆冷却）","5~30 kW（随功率、海拔升高）","已含在 η_fc 系统效率，不重复计"],
            ]} />
            <Tex block math="P_{aux}(T) \;=\; \mathrm{clip}\!\left(3 + 0.15\,|T-20^\circ\mathrm{C}|,\; 2,\; 8\right)\ \mathrm{kW}" />
            <Table head={["场景","附件功率","物理原因"]} rows={[
              ["常温 20℃","≈3 kW（基础负荷）","气泵、转向、低压电子设备"],
              ["夏天 35℃","≈5.3 kW","驾驶室空调 + 散热风扇功耗上升"],
              ["冬天 −15℃","≈8 kW（上限）","座舱 PTC 加热 + 电池/电堆保温"],
            ]} />
            <p className="hw-note">为什么不能忽略：一趟 500km 高速（约 7h）附件要吃掉 ≈1.7 kg 氢气，占总氢耗 5% 上下，极寒翻倍——不建模就漏账。训练端（ML）通过温度特征隐式学习附件影响，物理端显式用上式。</p>
          </Sec>

          <Sec num="10" title="端到端手算工作簿（附录 B 三路段，逐步）">
            <Table head={["段","路况","里程","均速","坡度","海拔(中点)","温度"]} rows={[
              ["1","高速平路","10 km","80 km/h","0%","50 m","20℃"],
              ["2","高速上坡","8 km","70 km/h","+3%","170 m（爬升 240m）","30℃"],
              ["3","高速下坡","6 km","75 km/h","−2%","230 m（下降 120m）","15℃"],
            ]} />
            <Table head={["步","段1 平路","段2 上坡","段3 下坡"]} rows={[
              ["空气密度 ρ(H)","1.219 kg/m³","1.205","1.198"],
              ["滚动阻力 F_roll","2646 N","2646","2646"],
              ["空气阻力 F_aero","791 N","1557 N","683 N"],
              ["坡度阻力 F_grade","0 N","8820 N（+3%）","−5880 N（−2%）"],
...
export default function TechTabs() {
  const [tab, setTab] = useState<'ml' | 'physics'>('ml')
  return (
    <div className="tech-tabs">
      <div className="tech-tab-nav">
        <button className={'tech-tab-btn' + (tab === 'ml' ? ' on' : '')} onClick={() => setTab('ml')}>🤖 机器学习</button>
        <button className={'tech-tab-btn' + (tab === 'physics' ? ' on' : '')} onClick={() => setTab('physics')}>🔋 物理模型</button>
      </div>
      <div className="tech-tab-body">
        {tab === 'ml' ? <MLTab /> : <PhysicsTab />}
      </div>
    </div>
  )
}
