          ...
          <Sec num="6" title="结果与评估（实测）">
            ...
            <Sub title="6.4 预测校准（均值 → 中位）">
              ...
              <Table head={["均速区间 (km/h)","预测−实测中位偏差 (kg/100km)"]} rows={[
                ["0–40","+1.6"], ["40–60","+0.9"], ["60–80","+0.3"], ["80+","+0.5"], ["整体","+1.2%（相对误差）"]
              ]} />
              <p><b>校准公式</b>（分段线性偏差修正，低速多减、高速少减）：</p>
              <Tex block math="h_{2,cal}=h_{2,pred}-bias(v)" />
              <Tex block math="bias(v)=\min\!\big(\max(1.8-0.02\,v,\;0),\;1.8\big)\quad\mathrm{kg/100km}" />
              <p>其中 v 为段均速（km/h）。效果：天安门→机场 30km 路线预测 10.84 → 9.93 kg/100km，整体更贴近实测中位。</p>
              <p className="hw-note">边界：① 这是<b>经验偏差修正</b>（来自 5 折 CV 的 out-of-sample 偏差），不是物理公式；② 只把"均值估计"拉回"中位水平"，不改变路段间的相对排序；③ 若训练数据分布变化，需重跑 ml/calib_check.py 重拟合 bias 曲线；④ 业务上如需"最坏情况"（续航保险），应看未校准值或高分位。</p>
            </Sub>
          </Sec>
