...
ho C_dA v_i^3\Delta t_i" />
          ...
          <Sec num="3" title="特征工程（20 维，两级）">
            <Sub title="3.1 第一级：新线路可直接获取（11 个）">
          ...
          <Sec num="6" title="结果与评估（实测）">
            <Sub title="6.1 交叉验证">
              <p><b>按行程分组 5 折 CV：R²=0.38（每折 R² 取平均），RMSE=0.048 kg/km</b>（目标中位 ≈5.12 kg/100km；2026-08-22 修正 haversine + 质量特征 + 里程加权聚合后重训；口径说明见 §6.4 末注）。此值使用真实 v 差分加速度，为诚实水平。</p>
            </Sub>
            ...
            <Sub title="7.2 计算链路（L2 四阻力 → L3 动力总成 → L4/L5 效率与氢耗）">
              ...
              <Tex block math="P_{drive}=\frac{P_{wheel}}{\eta_{mt}}+P_{aux},\qquad P_{fc}=\begin{cases}\min(P_{drive},180) & P_{drive}\ge 30\\ P_{drive} & 0&lt;P_{drive}&lt;30\\ 0 & P_{drive}\le 0\end{cases}\ \mathrm{kW},\qquad P_{bat}=P_{drive}-P_{fc}" />
