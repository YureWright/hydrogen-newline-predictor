      ...
      <div className="howitworks-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="howitworks-body">
          ...
          <Sec num="2" title="数据：来源与处理">
            ...
            <Sub title="2.4 段聚合">
              <p>按行程（相邻时间差 &gt;300s 视为新行程）把 60s 点按 5km 累计里程分桶 → <b>1066 个路段样本</b>（两车混合，过滤 h2_per_km∈(0.02,0.5) 且 len_km≥1 后）。每段统计：均速/坡度/海拔/温度/风速/湿度/道路等级/时段/段长 + 真实目标 kg/km。</p>
            </Sub>
            <Sub title="2.5 训练 vs 预测的数据来源口径差异（透明披露）">
              ...
              <p><b>已知口径差异（如实披露）</b>：① <b>天气源不同</b>——训练用 ERA5 历史再分析（模型回算值），预测用 QWeather 实时预报，同点同时刻可能差数度 / 数 m/s；② <b>道路等级来源不同</b>——regeo 关键词推断 vs OSM 地图匹配，个别路段等级可能不一致（如 S12 机场高速）；③ <b>里程口径不同</b>——GPS 直线累计 vs 高德路线里程（含道路曲率，通常略大），整体约 1% 量级；④ <b>载重未知（最大偏差源）</b>——训练数据无载重列，模型学到的是两车运营的"平均载重"水平，预测时无法体现空载/满载差异。</p>
            </Sub>
          </Sec>
