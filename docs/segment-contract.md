...
**长事件 step 拆分时的时长/速度分配**：一个 step 被拆成「主体段 + 尾部事件区」时，
主体段按整步均速、尾部按该事件的典型通过速度各算一份"名义耗时"，再统一缩放到高德给的整步时长。
因此 ① 拆分前后总时长完全相等；② 头尾速度比恒等于名义速度比，尾部一定慢于主体段；
③ 两段速度都被整步均速锚住，不会出现物理上不可能的值。
不能给尾段按固定速度独立计时后追加到主体段之后（全程时长会虚增 7%），
也不能把差额全部从主体段扣掉（误差会挤成 300+ km/h 的假高速段，而风阻 ∝ v²）。

**全程均速口径**：`summarizeSegments().avgSpeedKmh` = 总里程 ÷ 总时长（调和平均），
不是各段速度按里程加权的算术平均——后者会系统性高估（城区线路实测偏高 38%），
因为慢段占用的时间被低估了。

验证：`npm run verify:segment`（41 项纯函数 + 真实线路）+ `npm run verify:split`（行为区 + 坡度切分 + 启停口径 + 时长守恒 + 高程去噪 93 项断言）。
