...
| 红绿灯路口 | 减速→(停车)→启动 | **停车是概率事件 P≈0.4**（可配置） |
...
| 指标 | 优化前 | 优化后 |
| --- | --- | --- |
| 路段数 | 43 段（含 90km 长段、0.05km 碎段） | **约 295 段**（0.05~10km，行为区+坡度带切分） |
| 坡度分辨率 | 90km 平均 ≈ 0%（爬升下降抵消） | 京藏山区逐段可辨：**+5.44% / -5.86% / +6.22%** 独立上坡下坡 |
| 行为区 | 收费站/匝道被吞并 | 收费站、匝道、转弯、路口独立成段并挂概率 |
| 物理模型输入 | 粗 | 段内匀质（坡度幅度 ≤1.5% + 行为单一），启停按期望次数计入 |

> 该切分对“新线路氢耗预测”至关重要：企业 MATLAB 模型接入时直接消费细分后的 SegmentData，无需感知切分细节。验证：`npm run verify:split`（26 项纯函数断言）+ `npm run enrich`（真实线路）。
