...
| 动力总成（L3） | P_wheel=F·v；P_aux=clip(3+0.15·|T−20|, 2, 8) kW；P_drive=P_wheel/η_mt+P_aux；P_fc=三分支（≥30：min(P_drive,180)；0~30：最低 30kW 稳定运行；≤0 再生：最低 30kW 稳定运行，附件电由电堆出、回收电充电池）；P_bat=clip(P_drive−P_fc, −150, 150) kW（正=放电，负=充电；±150kW=75kWh×2C，超限由机械制动耗散） |
...
- **手算验证**：设计文档附录 B 三路段工作簿（平路/上坡/下坡）→ 0.660 / 1.236 / 0.144 kg，合计 **2.039 kg、8.50 kg/100km**，与 `ml/physics.py` 逐段完全一致（下坡段电堆保持最低稳定运行：附件电由电堆烧氢出、回收电充电池，故氢耗为小正值 0.144 kg）
- **实测锚点**：乌兰察布→天津 508km、39.7t → 物理模型 **7.26 kg/100km**（含风向修正；无风对照 7.28，本批数据风影响 ≈0.3%），落在官方满载锚点 7.1~8 范围内（本测载重 30t，接近满载工况）；机器学习 4.37（数据清洗后系统性低估，见知识库）——两者差异可作为数据质量信号
