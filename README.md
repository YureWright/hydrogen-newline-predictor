...
| **氢耗预测（机器学习）** | 用两辆 H49 重卡实车数据训练的段级模型：系统分段 → **工况合成**（按道路等级×均速从实车片段库拼接 60s v/a）→ 深度特征 → HistGB 预测每段氢耗，加总得线路总氢耗/百公里氢耗（R²≈0.50，按行程分组 CV）；「📖 技术原理」按钮打开原理页 |
...
系统分段只给"平均速度"，而氢耗对加减速/启停细节极敏感。本方案用两辆 H49 重卡 60s 实车数据建**工况片段库**（按 道路等级×均速 8 桶），新线路每段从对应桶拼接 v/a 片段、按均速对齐、按坡度调制，合成出行驶工况 → 算出加速/空阻/上坡能量、速度分位数、启停占比等深度特征 → HistGB 预测每段氢耗。真实目标用**氢气剩余量差分**（中位 ≈5.2 kg/100km，符合重卡实况），按行程分组 CV R²≈0.50。详见 `docs/氢耗ML建模调研_20260821.md` 与 `ml/README.md`。
