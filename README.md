...
| 动力总成（L3） | P_wheel=F·v；P_aux=clip(3+0.15·|T−20|, 2, 8) kW；P_drive=P_wheel/η_mt+P_aux；P_fc=clip(P_drive, 30, 180) kW；P_bat=P_drive−P_fc（正=放电，负=充电） |
...
- **实测锚点**：乌兰察布→天津 508km、39.7t → 物理模型 **6.82 kg/100km**（含风向修正；无风对照 6.85，本批数据风影响 ≈0.4%），略低于官方满载锚点 7.1~8（本测载重 30t、非满载 49t，属合理区间）；机器学习 4.37（数据清洗后系统性低估，见知识库）——两者差异可作为数据质量信号
- **前端**：测算面板「预测模型」下拉可选 机器学习 / 物理模型 / 双引擎对比；物理表 19 列全部中文名、点击表头排序、一键导出 CSV（含全部中间变量）
- **v1 简化（诚实披露）**：η_fc 固定 0.5、P_fc 为三分支控制（正常/低功率跟随/再生关闭）、暂忽略速度波动 σ 与动态 SOC——后续接企业模型数据后可替换 / 升级
