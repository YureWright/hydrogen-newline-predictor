...
| 动力总成（L3） | P_wheel=F·v；P_aux=clip(3+0.15·|T−20|, 2, 8) kW；P_drive=P_wheel/η_mt+P_aux；P_fc=三分支（≥30：min(P_drive,180)；0~30：最低 30kW 稳定运行；≤0 再生：0 关闭）；P_bat=clip(P_drive−P_fc, −150, 150) kW（正=放电，负=充电；±150kW=75kWh×2C，超限由机械制动耗散） |
...
- **手算验证**：设计文档附录 B 三路段工作簿（平路/上坡/下坡）→ 0.660 / 1.236 / 0.000 kg，合计 **1.896 kg、7.90 kg/100km**，与 `ml/physics.py` 逐段完全一致（下坡段再生策略：电堆关闭、电池吸收回收功率，故氢耗为 0）
- **实测锚点**：乌兰察布→天津 508km、39.7t → 物理模型 **6.89 kg/100km**（含风向修正、低速段电堆最低稳定运行；无风对照 6.91，本批数据风影响 ≈0.3%），略低于官方满载锚点 7.1~8（本测载重 30t、非满载 49t，属合理区间）；机器学习 4.37（数据清洗后系统性低估，见知识库）——两者差异可作为数据质量信号
- **前端**：测算面板「预测模型」下拉可选 机器学习 / 物理模型 / 双引擎对比；物理表 19 列全部中文名、点击表头排序、一键导出 CSV（含全部中间变量）
- **v1 简化（诚实披露）**：η_fc 固定 0.5、P_fc 三分支（正常/低速最低稳定/再生关闭）、电池充放电 ±150kW 限幅、暂忽略速度波动 σ 与动态 SOC（电池净放电未闭环）——后续接企业模型数据后可替换 / 升级
