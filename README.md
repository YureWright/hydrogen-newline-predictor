...
- **手算验证**：设计文档附录 B 三路段工作簿（平路/上坡/下坡）→ 0.660 / 1.235 / 0.144 kg，合计 **2.039 kg、8.50 kg/100km**，与 `ml/physics.py` 逐段完全一致
- **实测锚点**：乌兰察布→天津 508km、39.7t → 物理模型 **7.21 kg/100km**，落在官方 7.1~8 锚点附近；机器学习 4.37（数据清洗后系统性低估，见知识库）——两者差异可作为数据质量信号
- **前端**：测算面板「预测模型」下拉可选 机器学习 / 物理模型 / 双引擎对比；物理表 19 列全部中文名、点击表头排序、一键导出 CSV（含全部中间变量）
- **v1 简化（诚实披露）**：η_fc 固定 0.5、P_fc 直接截断、暂忽略速度波动 σ 与动态 SOC——后续接企业模型数据后可替换 / 升级
