...
4. **质量来源不同**：训练用行程级能量平衡反推（`ml/mass_est.py`，电机 V/I → 整趟行程能量守恒解 m，含 η/Crr/CdA 假设，系统偏差用官方锚点校准）；预测用用户输入的载重 + 整备。注意：训练数据只有部分行程能反推（车1 3 趟 / 车2 14 趟），其余用该车中位填充，因此**ML 模型的载重响应是离散档位、且个别场景（如城市高载重）可能不合理**——可靠的载重敏感性依赖物理模型（开发中）。
