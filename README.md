...
## 创新点：路段自适应细分（Grade-Aware Adaptive Segmentation）

**痛点**：高德驾车路线按“导航动作”切 step——高速上连续 90km 不拐弯只给 1 段、城区几米也给 1 段。若直接按 step 做氢耗仿真，一个 90km 段的坡度会被平均（先爬升后下降 → 平均≈0%），而重卡氢耗对坡度最敏感，结果失真；几米的碎段又无法计算坡度。

**方案**（在 DEM 高程填充阶段实现，纯数据层，不依赖高德）：

1. **合并碎段**：<0.3km 的段并入前一段，清掉起终点“向西行驶 128 米”这类垃圾行；
2. **按坡度变号细分长段**：>10km 的段沿 DEM 高程剖面，在**坡度变号点（山峰/山谷）**切开，每子段控制在 2~10km；
3. **属性继承 + 重算**：子段继承父段路名/等级/路况/停车密度，各自重新计算坡度、海拔、累计爬升/下降，并按比例校正里程使子段合计 = 父段里程。

**效果**（实测乌兰察布→天津 490km）：

| 指标 | 优化前 | 优化后 |
| --- | --- | --- |
| 路段数 | 43 段（含 90km 长段、0.05km 碎段） | 约 188 段（每段 2~10km） |
| 坡度分辨率 | 90km 平均 ≈ 0%（爬升下降抵消） | 逐段真实：+1.5% 爬坡 / -3.7% 陡下坡清晰可辨 |
| 物理模型输入 | 粗 | 细粒度，长上坡/长下坡分开计算，氢耗预测更准 |

> 该细分对“新线路氢耗预测”至关重要：企业 MATLAB 模型接入时直接消费细分后的 SegmentData，无需感知切分细节。
...
- [ ] 路段行为标注（收费站/路口/匝道/转弯：变速情况 + 变速概率）
- [ ] 切分算法升级（坡度变号 + 坡度带阈值自适应细分；短段不再合并）
