...
| 红绿灯路口 | 减速→(停车)→启动 | **停车是概率事件**：城市 3 路口/km、国道 0.4、省道 0.6（按里程线性折算，保留小数，段被切分后总数不变）；单路口 P=0.35(畅通)~0.95(严重拥堵)，随实时路况变化 |
| 高速出入口匝道 | 减速→并线→加速 | 明显减速 P≈0.85 |
| 急转弯（指令/航向角>40°） | 减速→转弯→加速 | 减速 P≈0.7 |
| 服务区/城市起停 | 减速→(停车)→启动 | 按密度折算 |

**L2 坡度自适应切分**（对 ≥1km 的**巡航段/城市起停段**，沿 DEM 剖面；收费站/匝道/转弯等事件段是点状行为，不参与地形切分）：
...
>
> **切分不改变总量**：路口数按里程线性折算（保留小数），一条路无论被地形切成几片，全程路口总数与期望停车次数都不变；长事件 step 拆出的“主体段 + 尾部事件段”时长之和 = 高德给的该 step 实测时长。
>
> 上表数字实测于事件计数/时长口径修正之前，段数与事件计数会有小幅变化，重跑 `npm run enrich` 可校准。
>
> 该切分对“新线路氢耗预测”至关重要：企业 MATLAB 模型接入时直接消费细分后的 SegmentData，无需感知切分细节。验证：`npm run verify:split`（64 项纯函数断言）+ `npm run verify:segment`（45 项）+ `npm run enrich`（真实线路）。
