...
与机器学习**同一数据接口**（SegmentData），但**不依赖训练数据**：从车辆动力学 + 燃料电池原理按公式逐步计算，每段输出 **15 个中文名中间变量**（车速/空气密度/滚动阻力/空气阻力/坡度阻力/总驱动力/轮边功率/附件功率/驱动电功率/电堆功率/电池功率/行驶时长/电堆效率/电堆电能/氢耗）+ 总氢耗 / 百公里氢耗。

| 环节 | 公式 / 说明 |
| --- | --- |
| 四阻力（L2） | F_roll=C_rr·m·g；F_aero=½ρ(H)·C_d·A·v²；F_grade=m·g·sinθ；F_acc=δ·m·a（匀速巡航为 0） |
...
| 效率与氢耗（L4/L5） | t=s/v；E_fc=P_fc·t；m_H₂=E_fc/(η_fc·LHV)，η_fc=0.5，LHV=33.3 kWh/kg |
...
- **手算验证**：设计文档附录 B 三路段工作簿（平路/上坡/下坡）→ 0.660 / 1.236 / 0.144 kg，合计 **2.040 kg、8.50 kg/100km**，与 `ml/physics.py` 逐段完全一致（下坡段电堆保持最低稳定运行：附件电由电堆烧氢出、回收电充电池，故氢耗为小正值 0.144 kg）
...
- **前端**：测算面板「预测模型」下拉可选 机器学习 / 物理模型 / 双引擎对比；物理表 19 列全部中文名、点击表头排序、一键导出 CSV（含全部中间变量）
- **v1 简化（诚实披露）**：η_fc 固定 0.5、P_fc 三分支（正常/低速最低稳定/再生最低稳定）、电池充放电 ±150kW 限幅、暂忽略速度波动 σ 与动态 SOC（电池净放电未闭环）——后续接企业模型数据后可替换 / 升级
